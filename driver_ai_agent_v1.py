...
def move_and_update_file(service, file_id, new_name, category, description=""):
    """移動檔案、重新命名，並寫入 AI 畫面描述到備註"""
    # 1. 找尋或創建分類資料夾
...
任務 1：畫面觀察與描述 (請詳細記錄文字、品牌、產品、價格)
任務 2：判斷【活動名稱】(若畫面無明確活動但檔名有暗示，請採納檔名。如：年節禮、母親節、日常活動等)
任務 3：判斷【用途】\n{purpose_prompt}
...
{{"畫面描述": "簡述內容與檔名線索", "活動名稱": "填入選項", "用途": "填入選項"}}"""
    ...
    try:
        ...
        return {"畫面描述": "辨識失敗", "活動名稱": "未分類", "用途": "未分類"}
            ...
            for file in files:
                ...
                try: # 🌟 新增的內部 try，專門保護單一檔案的處理過程
                    ...
                    if ext in ['.jpg', '.jpeg', '.png', '.pdf', '.ai']:
                        ...
                        new_name = f"{PRESET_YEAR}_{activity}_{safe_size}_{final_purpose}_{PRESET_DEVICE}_{date_str}{ext}"
                        category = activity

                        print(f"✅ 視覺分析完成! 判定活動: {activity} | 用途: {final_purpose}")
                    ...
                    # 【路線 B：純文字或文件】
                        ...
                        if ai_result:
                            ...
                            print(f"✅ 文檔分析完成! 分類: {category}")
                        else:
                            raise ValueError("AI 回傳空白結果")

                    # 執行 Drive 的重新命名、移動與更新備註
                    move_and_update_file(service, file_id, new_name, category, description)
                    print(f"🎯 歸檔成功: {new_name} -> [{category}] 資料夾")
